...
```
:::info
For React applications, environment variables must be prefixed with REACT_APP_ to be accessible in the client-side code.
:::
Use
```shell
export REACT_APP_ORY_SDK_URL=https://$PROJECT_SLUG.projects.oryapis.com
```
And in your code, reference it as:
```js
